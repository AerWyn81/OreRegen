    ...
    public void build() {
        if (player != null) {
            ...
            if (!isConsoleSender && command != null && size > pageHeight) {
                TextComponent c1 = new TextComponent(FormatUtils.translate(LanguageService.getMessage("Chat.PreviousPage")));
                c1.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/or " + command + " " + (pageNumber - 1)));
                ...
                TextComponent c2 = new TextComponent(FormatUtils.translate(LanguageService.getMessage("Chat.NextPage")));
                c2.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/or " + command + " " + (pageNumber + 1)));
